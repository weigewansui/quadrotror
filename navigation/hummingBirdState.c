...
void *aciThread(void)
{
	...
	while(1)
	{
		...
		if(var_getted) {
			...
			state_msg.rpm[0] = rpm1;
			state_msg.rpm[1] = rpm2;
			state_msg.rpm[2] = rpm3;
			state_msg.rpm[3] = rpm4;
...
void varListUpdateFinished(void) {
	...
	aciAddContentToVarPacket(0, 0x0100, &rpm1);
	aciAddContentToVarPacket(0, 0x0101, &rpm2);
	aciAddContentToVarPacket(0, 0x0102, &rpm3);
	aciAddContentToVarPacket(0, 0x0103, &rpm4);
